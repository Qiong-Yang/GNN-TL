...
>>>>>>> 6223ed9d9eb8ba67a0836fe2803ae89b19453aea
    ...
    def mlp(self, vectors):
        ...
        return outputs
    def forward_regressor(self, data_batch, train): #Variable(label.cuda())
        inputs =data_batch[:-1]
        a=data_batch[-1]
        a=[t.cuda() for t in a]
...
>>>>>>> 6223ed9d9eb8ba67a0836fe2803ae89b19453aea
    ...
    data=pd.read_csv('data/hilic_msfinder_export_pks.csv')
    x=list(data['SMILES'])
    y=list(data['RTs'])
    path='/data/'
    
    def random_split_data(ratio,s, X, Y):
        
        N=len(X)-round(len(X)*ratio)
        seed =s
        np.random.seed(seed) 
        ...
        for i in range(len(train_indices)):
            X_train.append(X[train_indices[i]])
            Y_train.append(Y[train_indices[i]])
        for i in range(len(test_indices)):
            X_test.append(X[test_indices[i]])
            Y_test.append(Y[test_indices[i]])
        dataset_train= pp.create_dataset_randomsplit(X_train,Y_train,datafile)
        dataset_train, dataset_dev = split_dataset(dataset_train, 0.9)
        dataset_test= pp.create_dataset_randomsplit(X_test,Y_test,datafile) 
        return dataset_train, dataset_dev,dataset_test
    dataset_train, dataset_dev,dataset_test=random_split_data(0.1,22,x, y)
    ...
    print('-'*100)
    file_MAEs = path+'output/'+'_MAEs'+'.txt'
    file_test_result  = path+'output/'+ '_test_prediction'+ '.txt'
    file_predictions = path+'output/'+'_train_prediction' +'.txt'
    file_model = path+ 'output/'+'_model'+'.h5'
    ...
    plt.xlabel('Epoch')
    plt.legend()
    plt.savefig(path+'output/'+'-MAE.tif',dpi=300)
    plt.show()
    ...
    plt.savefig(path+'output/'+'PC-test.tif',dpi=300)
    plt.show()
